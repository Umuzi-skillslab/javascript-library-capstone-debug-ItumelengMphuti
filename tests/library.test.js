...
describe("Book Class", () => {
  ...
  test("should return book infomation", () => {
    ...
    expect(book.getInfo()).toBe(
      '"Fake Book" by John Doe (2002) - ISBN: 978-0-7432-7356-5 | Available: 3/3',
    );
  });
  ...
  test("should allow multpiple members to checkout copies", () => {
    ...
    expect(book.checkedOut[0].checkoutDate).toEqual(new Date("2026-07-06"));
    expect(book.checkedOut[1].checkoutDate).toEqual(new Date("2026-07-06"));
  });
  ...
  test("standard members cannot reserve books", () => {
    const member = new Member("001", "John", "john@email.com", "standard");

    expect(member.canReserve()).toBe(false);
  });

  test("should not reserve the same book twice", () => {
    const book = new Book("ISBN1", "Book", "Author", 2020, 0);
...
describe("Member Class", () => {
  ...
  test("should return the correct membership duration in days", () => {
    ...
    expect(member.getMembershipDuration()).toBe(
      "Member has been active for 5 days.",
    );
  });
  test("should return the correct membership duration in months and days", () => {
    ...
    expect(member.getMembershipDuration()).toBe(
      "Member has been active for 5 months and 4 days.",
    );
  });
  test("should return the correct membership duration in years", () => {
    ...
    expect(member.getMembershipDuration()).toBe(
      "Member has been active for 2 years, 1 month and 4 days.",
    );
  });
  test("should return an error message for a future join date", () => {
    ...
    expect(() => member.getMembershipDuration()).toThrow(
  ...
  test("should get correct member information", () => {
    ...
    expect(member.getMemberInfo()).toBe(
      "John Doe (1) - standard member | johndoe@email.com",
    );
  });
  ...
  test("should return false when a member downloaded 5 books", () => {
    ...
    member.downloadedBooks = ["ISBN1", "ISBN2", "ISBN3", "ISBN4", "ISBN5"];
...
describe("PremiumMember Class", () => {
  ...
  test("premium members can reserve books", () => {
    const member = new PremiumMember("001", "Jane", "jane@email.com");
...
describe("Array Operations", () => {
  test("should calculate total late fees using reduce", () => {
    const memberRecord = {
      overdueBooks: [{ daysLate: 2 }, { daysLate: 3 }],
    };
  ...
  test("should combine book collections using spread", () => {
    ...
    expect(combineBookCollections(fiction, nonFiction, reference)).toEqual([
      "Book 1",
      "Book 2",
      "Book 3",
    ]);
...
describe("initializeUI", () => {
  test("should log an error when required elements are missing", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith("UI elements not found.");
...
describe("DOM Manipulation", () => {
  ...
  test("should display the correct title", () => {
    ...
    expect(document.body.textContent).toContain("Things Fall Apart");
  });
  ...
  test("should display the author", () => {
    ...
    expect(document.body.textContent).toContain("James Clear");
  });
  ...
  test("should capitalize the book type", () => {
    ...
    expect(document.body.textContent).toContain("Ebook");
  });
});
...
describe("JSON Operations", () => {
  beforeEach(() => {
    ...
    members.push(new Member("001", "John Doe", "john@test.com", "standard"));
  });
...
describe("displayBookDetails", () => {
  ...
  test("should display the book title", () => {
    ...
    expect(document.getElementById("book-details").textContent).toContain(
      "Atomic Habits",
    );
  });
  ...
  test("should display available copies", () => {
    ...
    expect(document.getElementById("book-details").textContent).toContain("5");
  });
  ...
  test("should not throw when book does not exist", () => {
...
describe("updateStatisticsDisplay", () => {
  beforeEach(() => {
    ...
    books.push(
      new Book("111", "Book One", "Author One", 2024, 3, "fiction", "physical"),
    );
  ...
  test("should display the total number of books", () => {
    ...
    expect(document.getElementById("total-books").textContent).toBe("2");
  });

  test("should display the total number of members", () => {
    updateStatisticsDisplay();

    expect(document.getElementById("total-members").textContent).toBe("1");
  });

  test("should display zero borrowed books initially", () => {
    updateStatisticsDisplay();

    expect(document.getElementById("books-borrowed").textContent).toBe("0");
  });
  ...
  test("should update borrowed books after a checkout", () => {
    ...
    expect(document.getElementById("books-borrowed").textContent).toBe("1");
  });
...
describe("renderMemberList", () => {
  ...
  test("should render one premium member", () => {
    members.push(
      new PremiumMember("002", "Jane Doe", "jane@test.com", "premium"),
    );
    ...
    expect(container.innerHTML).toContain("premium");
  });

  test("should render multiple members", () => {
    members.push(new Member("001", "John", "john@test.com", "standard"));

    members.push(new PremiumMember("002", "Jane", "jane@test.com", "premium"));
  ...
  test("should not throw when member-list container does not exist", () => {
    ...
    expect(() => renderMemberList()).not.toThrow();
  });
});
...
describe("handleBookClick", () => {
  beforeEach(() => {
    ...
    books.push(
      new Book("123", "Test Book", "Author", 2024, 2, "fiction", "physical"),
    );
  });
  ...
  test("should display book details when clicking the book card", () => {
    ...
    expect(document.getElementById("book-details").innerHTML).toContain(
      "Test Book",
    );
  });
...
describe("handleFilterChange", () => {
  ...
  test("should display all books", () => {
    ...
    expect(document.querySelectorAll(".book-card").length).toBe(3);
  });
  ...
  test("should filter fiction books", () => {
    ...
    expect(catalogue.innerHTML).toContain("Things Fall Apart");

    expect(document.querySelectorAll(".book-card").length).toBe(1);
  });
  ...
  test("should filter non-fiction books", () => {
    ...
    expect(catalogue.innerHTML).toContain("Sapiens");
  ...
  test("should filter reference books", () => {
    ...
    expect(catalogue.innerHTML).toContain("Clean Code");
  ...
  test("should render no books for an invalid category", () => {
    ...
    expect(document.querySelectorAll(".book-card").length).toBe(0);
  });
});
...
describe("loadCatalogue", () => {
  ...
  test("should load physical books", async () => {
    ...
    expect(books[0].title).toBe("Things Fall Apart");
  });
  ...
  test("should load digital books", async () => {
    ...
    expect(books[0].fileSize).toBe("5MB");
  ...
  test("should load both physical and digital books", async () => {
    ...
    expect(books[1]).toBeInstanceOf(DigitalBook);
  });
  ...
  test("should render the catalogue after loading", async () => {
    ...
    expect(document.getElementById("catalogue-list").innerHTML).toContain(
      "Rendered Book",
    );
  });
...
describe("setupEventListeners", () => {
  ...
  test("should attach the search listener", async () => {
    const spy = jest.spyOn(HTMLInputElement.prototype, "addEventListener");

    await initializeUI();

    expect(spy).toHaveBeenCalledWith("input", handleSearch);
  });

  test("should attach the category filter listener", async () => {
    const spy = jest.spyOn(HTMLSelectElement.prototype, "addEventListener");

    await initializeUI();

    expect(spy).toHaveBeenCalledWith("change", handleFilterChange);
  });

  test("should attach the borrow form listener", async () => {
    const spy = jest.spyOn(HTMLFormElement.prototype, "addEventListener");

    await initializeUI();

    expect(spy).toHaveBeenCalledWith("submit", handleBorrowSubmit);
  });

  test("should attach the catalogue click listener", async () => {
    const spy = jest.spyOn(HTMLDivElement.prototype, "addEventListener");

    await initializeUI();

    expect(spy).toHaveBeenCalledWith("click", handleBookClick);
  });

  test("should attach navigation listeners", async () => {
    const spy = jest.spyOn(HTMLButtonElement.prototype, "addEventListener");
...
`;
  ...
  test("should show an error when inputs are empty", () => {
    ...
    expect(messageBox.textContent).toBe(
      "Please enter both a Member ID and an ISBN.",
    );
  ...
  test("should show member not found", () => {
    ...
    expect(messageBox.textContent).toBe("Member '999' does not exist.");
  ...
  test("should show book not found", () => {
    ...
    expect(messageBox.textContent).toBe("Book with ISBN '999' was not found.");
  ...
  test("should show a success message when borrowing succeeds", () => {
    ...
    expect(messageBox.textContent).toBe(
      '"Test Book" has been borrowed successfully.',
    );
...
describe("showMessage", () => {
  ...
  test("should display a success message", () => {
    ...
    expect(messageBox.textContent).toBe("Book borrowed successfully!");

    expect(messageBox.className).toContain("success");

    expect(messageBox.classList.contains("hidden")).toBe(false);
  });
  ...
  test("should display an error message", () => {
    ...
    expect(messageBox.textContent).toBe("Member not found.");
  ...
  test("should use info as the default type", () => {
    ...
    expect(messageBox.textContent).toBe("Welcome!");

    expect(messageBox.className).toContain("info");
  });
  ...
  test("should hide the message after 5 seconds", () => {
    ...
    expect(messageBox.classList.contains("hidden")).toBe(true);
  });
...
describe("createMemberForm", () => {
  ...
  test("should render the member form", () => {
    ...
    expect(document.getElementById("new-member-form")).not.toBeNull();
  });
  ...
  test("should require all fields", () => {
    ...
    document.getElementById("new-member-form").dispatchEvent(
      new Event("submit", {
        bubbles: true,
        cancelable: true,
      }),
    ...
    expect(messageBox.textContent).toBe("Please complete all fields.");
  ...
  test("should create a standard member", () => {
    ...
    expect(messageBox.innerHTML).toContain("Member added successfully!");

    expect(messageBox.innerHTML).toContain("Member ID: 001");

    expect(messageBox.innerHTML).toContain("Name: John");

    expect(messageBox.innerHTML).toContain("Membership: standard");
  ...
  test("should create a premium member", () => {
    ...
    expect(messageBox.innerHTML).toContain("Name: Jane");

    expect(messageBox.innerHTML).toContain("Membership: premium");
...
describe("returnBook", () => {
  beforeEach(() => {
    ...
    const member = new Member("001", "John", "john@test.com", "standard");
  ...
  test("should successfully return a borrowed book", () => {
    ...
    expect(result.message).toBe('"Test Book" has been returned successfully.');
  ...
  test("should fail when member does not exist", () => {
    ...
    expect(result.message).toBe("Member '999' does not exist.");
  });
  ...
  test("should fail when book does not exist", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.message).toBe("Book with ISBN '999' was not found.");
  });
  ...
  test("should fail if member never borrowed the book", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.message).toBe("This member has not borrowed this book.");
  });
});
...
describe("handleReturnSubmit", () => {
  ...
  test("should show validation message when inputs are empty", () => {
    ...
    expect(feedback.textContent).toBe("Please enter both Member ID and ISBN.");
  });
  ...
  test("should return a borrowed book", () => {
    ...
    expect(feedback.textContent).toBe(
      '"Test Book" has been returned successfully.',
    );
...
describe("countBooksRecursive", () => {
  test("should return 0 for an empty array", () => {
    expect(countBooksRecursive([])).toBe(0);
  });

  test("should count all books recursively", () => {
    const bookList = [
      new Book("1", "Book 1", "Author", 2020, 2, "fiction", "physical"),
      new Book("2", "Book 2", "Author", 2021, 3, "fiction", "physical"),
      new Book("3", "Book 3", "Author", 2022, 1, "fiction", "physical"),
    ];

    expect(countBooksRecursive(bookList)).toBe(3);
  });

  test("should return 0 for invalid input", () => {
    expect(countBooksRecursive(null)).toBe(0);
  });

  test("should return 0 for undefined input", () => {
    expect(countBooksRecursive(undefined)).toBe(0);
  });
});

describe("calculateTotalCopies", () => {
  test("should calculate the total copies of multiple books", () => {
    ...
    expect(calculateTotalCopies(book1, book2)).toBe(8);
  });

  test("should return 0 when no books are provided", () => {
    expect(calculateTotalCopies()).toBe(0);
  });

  test("should calculate copies for a single book", () => {
    const book = new Book(
      "1",
      "Book 1",
      "Author",
    ...
    expect(calculateTotalCopies(book)).toBe(7);
  });
});
